...
function Container({data}) {
  ...
  useEffect(() => {
    ...
    useEffect(() => {
      const getToken = async() => {
        try {
          const {data: {token: returnedToken}} = axios.get(`${GET_CALL_TOKEN}/${userInfo.id}`)
        ...
        import("zego-express-engine-webrtc").then( async({ZegoExpressEngine})=> {
          ...
          zg.on("roomStreamUpdate", async(roomID, updateType, streamList, extendedData) => {
            if(updateType === "ADD"){
              ...
              zg.logoutRoom(data.roomId.toSring())  
          ...
          await zg.loginRoom(
            data.roomId.toSring(), 
            token, 
            {userId: userInfo.id.toSring(), userName:userInfo.name}, 
            {userUpdate: true}
          )
  ...
  const endCall = () => {
    ...
    if (zgVar && localStream && publicStream){
      ...
      zgVar.logoutRoom(data.roomId.toSring())
    }

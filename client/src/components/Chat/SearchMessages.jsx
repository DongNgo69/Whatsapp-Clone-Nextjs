...
function SearchMessages() {
  ...
  useEffect(() => {
    if (searchTerm) {
      setSearchedMessages(
        messages.filter(
          (message) => 
            message.type === "text" && message.message.toLowerCase().includes(searchTerm)
        )
      )
    } else {
      setSearchedMessages([])
  ...
  return (
    <div className="border-conversation-border border-1 w-full bg-conversation-panel-background">
      <div className="h-16 px-4 py-5 flex gap-10 items-center bg-panel-header-background">
        <IoClose 
          className="cursor-pointer text-icon-lighter text-2xl"
          onClick={() => dispatch({type: reducerCases.SET_MESSAGE_SEARCH})}
        />
        <span className="text-white">Tìm tin nhắn</span>
      </div>
      <div className="overflow-auto custom-scrollbar h-full">
        <div className="flex items-center flex-col w-full">
          ...
          <span className="mt-10 text-secondary">
            {!searchTerm.length && `Tìm lại tin nhắn cũ với ${currentChatUser.name}`}
          </span>
        </div>
